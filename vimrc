...
set clipboard+=unnamed          " Use the system clipboard when copy/pasting using the default 'y' & 'p' commands
...
" Press <Ctrl+h> to search/replace the current visual selection
vnoremap <C-h> "hy:%s/<C-r>=substitute(@h,"\n",'\\n','g')<CR>//gc<left><left><left>
